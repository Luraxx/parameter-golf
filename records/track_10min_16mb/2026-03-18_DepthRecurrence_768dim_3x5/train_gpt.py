...
class Hyperparameters:
    ...
    # Model shape – depth recurrence: num_unique_layers blocks repeated to fill num_layers effective depth.
    vocab_size = int(os.environ.get("VOCAB_SIZE", 1024))
    num_layers = int(os.environ.get("NUM_LAYERS", 15))  # effective depth
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.eff_attn_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(model_dim, dtype=torch.float32)) for _ in range(n_eff)]
        )
        self.eff_mlp_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(model_dim, dtype=torch.float32)) for _ in range(n_eff)]
        )
        self.eff_resid_mixes = nn.ParameterList(
            [nn.Parameter(torch.stack((torch.ones(model_dim), torch.zeros(model_dim))).float()) for _ in range(n_eff)]
        )
...
def main() -> None:
    ...
    # - matrix params in shared transformer blocks use MATRIX_LR via Muon
    # - vectors/scalars (including per-effective-layer scales) use SCALAR_LR via Adam
    ...
    # Per-effective-layer adaptive parameters
    for p in base_model.eff_attn_scales.parameters():
        scalar_params.append(p)
    for p in base_model.eff_mlp_scales.parameters():
        scalar_params.append(p)
    for p in base_model.eff_resid_mixes.parameters():
        scalar_params.append(p)
